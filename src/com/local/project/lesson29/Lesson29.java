...
//        ПУЛ ГИБКОГО РАЗМЕРА (указываем минимальное и максимальное количество потоков)
        ...
        // КЭШИРУЮЩИЙ ПУЛ ПОТОКОВ  (Предполаегается, что свободные потоки все-таки есть, но чаще работает один поток, т.к. поток ожидает всего 60 сек, закрывается и создается новый поток)
        ...
        // ПУЛ НА ОДИН ПОТОК и одну задачу
...
//            result = taskResultContainer.get();
        ...
        // ПУЛ НА ОДИН ПОТОК и несколько задач
...
//        ForkJoinPool forkJoinPool = new ForkJoinPool(); можно создать двумя способами.
        ...
        scheduledExecutorService01.scheduleAtFixedRate(()->{  //метод принимает задачу, которую нужно выполнять через определенный интервал времени
            // Runnable
